...
int main(int argc, char ** argv) {
    ...
    if (sr_params.flags != 0 && grammar != NULL) {
        LOG_TEE("%s : Sorry, can't handle grammar when seqrep rewind mode is enabled\n", __func__);
        return 1;
    }
    // seqrep_sampler_params_dump(&sr_params);
#endif
...
#ifndef LLAMA_NO_SEQREP_SAMPLER
    size_t high_water_mark = prompt_tokens.size();
    std::vector<char> rewind_token_text_buf(128, 0);
    struct seqrep_rewind_state rewind_state(llama_n_vocab(ctx), n_ctx, 2000, prompt_tokens.size() - 1);

    rewind_state.set_logits_slot(ctx, prompt_tokens.size() - 1);
#endif
    ...
    while (n_remain > 0 && !interrupted) {
        ...
        const size_t rewind_distance =
            llama_seqrep_handle_rewind(
                ctx, rewind_state, last_tokens, prompt_tokens.size(),
                params.seqrep_params, &high_water_mark);

        if (rewind_distance > 0) {
            const size_t idx = last_tokens.size() - rewind_distance;
            const llama_token nl_id = llama_token_nl(ctx);

            console::set_display(console::error);
            fputs("\u3010", stdout);
            for (size_t i = idx; i < last_tokens.size(); i++) {
                if (last_tokens[i] == nl_id) {
                    fputs("\\n", stdout);
                    continue;
                }
...
#ifndef LLAMA_NO_SEQREP_SAMPLER
        rewind_state.set_logits_slot(ctx, last_tokens.size() - 1);
#endif
    }
